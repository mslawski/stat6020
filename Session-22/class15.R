...
##############################################################################
# [7] Gibbs Sampler for Bayesian Linear Regression
##############################################################################

# read-in features (X) and response (y)
diabetes_X <- read.table("pima.txt", header = TRUE)
diabetes_y <- read.csv("pimaY.txt", header = FALSE)

X <- as.matrix(diabetes_X)
...
# center y, so intercept can be dropped 
y <- as.numeric(diabetes_y[,2])
y <- y - mean(y)


### Gibbs sampling

# specificy number of samples and burn-in period
nsamples <- 1E4
burnin <- 1E3

# allocate space for storing the samples
Beta_samples <- matrix(nrow = nsamples, ncol = d)
tausq_inv_samples <- numeric(nsamples)
sigmasq_inv_samples <- numeric(nsamples)

# starting values for the sampler 
lm0 <- lm(y ~ X - 1)
betacur <- coef(lm0)
sigmasq_inv_cur <- 1/mean(residuals(lm0)^2)

# for efficienct beta-updates, compute SVD of X prior to starting the chain 
svdX <- svd(X)
U <- svd(X)$u
D <- svd(X)$d
V <- svd(X)$v
Uty <- crossprod(U, y)
# sum((X - U %*% diag(D) %*% t(V))^2) -- check

set.seed(222)

# start with the burn-in period
for(i in 1:burnin){

   # tausq_inv update
   tausq_inv_cur <- rgamma(1, shape = d/2, rate = sum(betacur^2)/2)
   
    
   # beta update using SVD
   lambda <- tausq_inv_cur / sigmasq_inv_cur 
   R1 <- sweep(V, D/(D^2 + lambda), FUN = "*", MARGIN = 2) 
   mu_beta <- R1 %*% Uty
   R2 <- sweep(V, sqrt(1/(D^2 + lambda)), FUN = "*", MARGIN = 2) /sqrt(sigmasq_inv_cur) # note that R2 %*% t(R2) equals the covariance matrix of the full conditional of beta
   betacur <- mu_beta + R2 %*% rnorm(d) 
   
    
   # sigmsq_inv update
   rss <- sum((y - X %*% mu_beta)^2)
   sigmasq_inv_cur <- rgamma(1, shape = n/2, rate = rss/2)
  
}

# now repeat and store 
    
for(i in 1:nsamples){

   # tausq_inv update
   tausq_inv_cur <- rgamma(1, shape = d/2, rate = sum(betacur^2)/2)
   tausq_inv_samples[i] <- tausq_inv_cur
   ...
   R2 <- sweep(V, sqrt(1/(D^2 + lambda)), FUN = "*", MARGIN = 2) /sqrt(sigmasq_inv_cur) # note that R2 %*% t(R2) equals the covariance matrix of the full conditional of beta
   betacur <- mu_beta + R2 %*% rnorm(d) 
   Beta_samples[i,] <- betacur
    
   # sigmsq_inv update
   rss <- sum((y - X %*% mu_beta)^2)
   sigmasq_inv_cur <- rgamma(1, shape = n/2, rate = rss/2)
   sigmasq_inv_samples[i] <- sigmasq_inv_cur 
}

# diagnostics 
par(mfrow = c(2,3))
plot(1/sigmasq_inv_samples, type = "l", main = expression(sigma^2))
plot(1/tausq_inv_samples, type = "l", main = expression(tau^2))
plot(Beta_samples[,1], type = "l", main = expression(beta[1]))

acf(1/sigmasq_inv_samples, main = expression(sigma^2), plot = TRUE)
acf(1/tausq_inv_samples,  main = expression(tau^2), plot = TRUE)
acf(Beta_samples[,1], main = expression(beta[1]),  plot = TRUE)

# results
par(mfrow = c(1,1))
index <- 3
hist(Beta_samples[,index], nclass = 50, main = colnames(X)[index], prob = TRUE)

# all coefficients (boxplots) 

index <- 1:10
boxplot(Beta_samples[,index], horizontal = TRUE, names = colnames(X)[index])
abline(v = 0, lty = "dashed", col = "blue")
points(coef(glmnet_cv)[index+1], index, col = "green")

# variance components
par(mfrow = c(3,1))
hist(sqrt(1/sigmasq_inv_samples), nclass = 50, main = expression(sigma), prob = TRUE)
hist(1/tausq_inv_samples, nclass = 50, main = expression(tau^2), prob = TRUE)
hist(tausq_inv_samples * sigmasq_inv_samples, nclass = 50, main = expression(sigma^2/tau^2), prob = TRUE)

# comparison with glmnet solution (ridge regression)
library(glmnet) 

par(mfrow = c(1,1))
glmnet_out <- glmnet(X, y, alpha = 0, standardize = FALSE, intercept = FALSE)
glmnet_cv <- cv.glmnet(X, y, alpha = 0, standardize = FALSE, intercept = FALSE)
...
index <- 333
xnew <- X[index,]#apply(X, 2, median)
ynew <- rnorm(n = nsamples, mean = Beta_samples %*% xnew, sd = sqrt(1/sigmasq_inv_samples))  
hist(ynew, nclass = 50, prob = TRUE, main = "posterior predictive distribution")

##############################################################################
# [8] Gibbs Sampler for Bayesian Linear Classification
##############################################################################

# data: spambase data set from the UCI machine learning repository 
spam_ytrain <- as.numeric(read.csv("spam_Ytrain.csv", header = FALSE)$V1)
spam_Xtrain <- as.matrix(read.csv("spam_Xtrain.csv", header = FALSE))
spam_ytest <- as.numeric(read.csv("spam_Ytest.csv", header = FALSE)$V1)
spam_Xtest <- as.matrix(read.csv("spam_Xtest.csv", header = FALSE)) 

y <- ifelse(spam_ytrain == 1, 1, 0)
X <- scale(spam_Xtrain, center = TRUE, scale = TRUE)    
n <- nrow(X)
d <- ncol(X)

# SVD computation for rapid updates:
svdX <- svd(X)
U <- svd(X)$u
D <- svd(X)$d
V <- svd(X)$v

# for efficient sampling from the truncated Normal distribution
library(truncnorm) 

nsamples <- 1E4
burnin <- 1E3

beta0_samples <- numeric(nsamples)
Beta_samples <- matrix(nrow = nsamples, ncol = d)
tausq_inv_samples <- numeric(nsamples)
...
# starting values for the sampler 
glm0 <- glm(y ~ X, family = binomial(link = 'probit'))
beta0cur <- -coef(glm0)[1]
betacur <- -coef(glm0)[-1]

# for efficienct beta-updates, compute SVD of X prior to starting the chain 
svdX <- svd(X) # not: intercept appended
U <- svd(X)$u
D <- svd(X)$d
V <- svd(X)$v
# sum((X - U %*% diag(D) %*% t(V))^2) -- check
...
for(i in 1:burnin){
   ...
   # z-update
   mu <- X %*% betacur + beta0cur
   z <- numeric(n)
   z[y == 1] <- rtruncnorm(sum(y == 1), a = 0, mean = mu[y == 1])
   z[y == 0] <- rtruncnorm(sum(y == 0), b = 0, mean = mu[y == 0]) 
    
   # beta update using SVD
   lambda <- tausq_inv_cur 
   R1 <- sweep(V, D/(D^2 + lambda), FUN = "*", MARGIN = 2) 
   mu_beta <- R1 %*% crossprod(U, z)
   R2 <- sweep(V, sqrt(1/(D^2 + lambda)), FUN = "*", MARGIN = 2) # note that R2 %*% t(R2) equals the covariance matrix of the full conditional of beta
   betacur <- mu_beta + R2 %*% rnorm(d)
   beta0cur <- mean(z) + rnorm(1, mean = 0, sd = 1/sqrt(n)) 
...
par(mfrow = c(2,3))
plot(1/tausq_inv_samples, type = "l", main = expression(tau^2))
plot(beta0_samples, type = "l", main = expression(beta[0]))
plot(Beta_samples[,1], type = "l", main = expression(beta[1]))

acf(1/tausq_inv_samples, main = expression(tau^2), plot = TRUE)
acf(beta0_samples,  main = expression(beta[0]), plot = TRUE)
acf(Beta_samples[,1], main = expression(beta[1]),  plot = TRUE)

### assess misclassification rates (training set) 

errs_train <- sapply(1:nsamples, function(z) mean((X %*% Beta_samples[z,] + beta0_samples[z] > 0) != y))

summary(errs_train)  # training error rates range between .06522 and .08913

hist(errs_train, nclass = 50, prob = TRUE)

### assess misclassification rates (test set)
    
ytest <- ifelse(spam_ytest == 1, 1, 0)
Xtest <- scale(spam_Xtest, center = TRUE, scale = TRUE)    

errs_test <- sapply(1:nsamples, function(z) mean((Xtest %*% Beta_samples[z,] + beta0_samples[z] > 0) != ytest))

summary(errs_test) 

hist(errs_test, nclass = 50, prob = TRUE)

# test errors are slightly higher than training errors, but noticeably so: 
summary(errs_test - errs_train) 

# 
# boxplot(Beta_samples, horizontal = TRUE)

# assess predictions (training set)

Pred <- apply(sapply(1:nsamples, function(z) X %*% Beta_samples[z,] + beta0_samples[z]), 1, range)

par(mfrow = c(1,1))
plot(rep(1:n, each = 2), plogis(c(Pred)), type = "n", xlab = "observation index", ylab = "P(y = 1|x)")
# takes some time to build this plot
sapply(1:n, function(z) lines(c(z, z), c(plogis(Pred[1,z]), plogis(Pred[2,z])), col =y[z] + 1))
