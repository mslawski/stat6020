...
%%% binary classification example (revisited) 
n = 1000;
x = 2*randn(n, 2) - 1;
y = (sign(x(:,1).*x(:,2)) + 1)/2;
d = size(x,2);
...
% numerical derivative check: useful 
%DSigma2(1.4)
%delta = 0.001;
...
while iter < maxiter
    ...
    % sample batch of size = 20
    batch = randperm(n);
    batch = batch(1:batchsize);
    
    % compute gradient (back propagation)
    Dloss = @(yhat) -(y(batch)./yhat - (1-y(batch))./(1 - yhat));
    dL = Dloss(yhat(batch));
    dsigma2 = Dsigma2(z2cur(batch));
    weights = (dL .* dsigma2);
    grad_wtilde = (Htilde(batch,:)' *  weights/ batchsize) + lambda * [0; wtilde(2:(L+1))];
    
    grad_Wtilde_p1 = (repmat(weights, [1 L]) .* Dsigma1(Z1cur(batch,:))) .* (repmat(wtilde(2:(L+1)), [1 batchsize])');
    %grad_Wtilde_p2 =  Xtildet_bdiag  * grad_Wtilde_p1(:);
    
    grad_Wtilde_p2 = kron(eye(L), Xtilde(batch,:)') * grad_Wtilde_p1(:);
    grad_Wtilde = reshape(grad_Wtilde_p2, [(d+1) L])/batchsize + lambda * [zeros(1, L);Wtilde(2:end,:)];
    % gradient update 
